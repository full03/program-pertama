...
if ($tanggal_awal_filter && $tanggal_akhir_filter) {
    ...
    // PERBAIKAN: Query hanya mengambil data yang ada di database
    $query_absensi = "
    SELECT 
        a.siswa_id,
        s.nokartu,
    ...
    $query_absensi .= " ORDER BY a.siswa_id, a.tanggal, a.jam_urutan";
    ...
    if ($result_absensi) {
        while ($row = mysqli_fetch_assoc($result_absensi)) {
            ...
            // Simpan tanggal yang terpakai
            if (!in_array($row['tanggal'], $tanggal_terpakai)) {
                $tanggal_terpakai[] = $row['tanggal'];
            }
            ...
            if (!isset($absensi_data[$key])) {
                $absensi_data[$key] = [
                    ...
                    'jam_absen' => null,
                    'jam_urutan' => 0,
                    'all_data' => [] // Menyimpan semua data per jam
                ];
            }
            
            // Simpan data per jam
            $absensi_data[$key]['all_data'][] = $row;
            ...
            // Update keterangan dan status (ambil yang terakhir)
            if ($row['keterangan']) {
...
// Urutkan tanggal terpakai
sort($tanggal_terpakai);
...
// ===== PROSES TAMBAH/UPDATE PENGATURAN JAM =====
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    $action = $_POST['action'];
    
    if ($action == 'add') {
        $nama_jam = mysqli_real_escape_string($conek, $_POST['nama_jam']);
        $jam_mulai = mysqli_real_escape_string($conek, $_POST['jam_mulai']);
        $jam_selesai = mysqli_real_escape_string($conek, $_POST['jam_selesai']);
        $batas_telat = mysqli_real_escape_string($conek, $_POST['batas_telat']);
        $urutan = mysqli_real_escape_string($conek, $_POST['urutan']);
        
        $query = "INSERT INTO pengaturan_jam_absen (nama_jam, jam_mulai, jam_selesai, batas_telat, urutan) 
                  VALUES ('$nama_jam', '$jam_mulai', '$jam_selesai', '$batas_telat', '$urutan')";
        mysqli_query($conek, $query);
        
    } elseif ($action == 'edit') {
        $id = mysqli_real_escape_string($conek, $_POST['id']);
        $nama_jam = mysqli_real_escape_string($conek, $_POST['nama_jam']);
        $jam_mulai = mysqli_real_escape_string($conek, $_POST['jam_mulai']);
        $jam_selesai = mysqli_real_escape_string($conek, $_POST['jam_selesai']);
        $batas_telat = mysqli_real_escape_string($conek, $_POST['batas_telat']);
        ...
        $query = "UPDATE pengaturan_jam_absen SET 
                  nama_jam='$nama_jam', 
                  jam_mulai='$jam_mulai', 
        ...
        $id = mysqli_real_escape_string($conek, $_POST['id']);
        $query = "DELETE FROM pengaturan_jam_absen WHERE id='$id'";
        mysqli_query($conek, $query);
    }
...
<head>
    ...
    * {
    ...
    .modal-header {
        ...
        border-bottom: 2px solid rgba(72, 49, 212, 0.08);
    }
    ...
    .jam-item {
        ...
        border-radius: 8px;
        margin-bottom: 0.3rem;
        border: 1px solid rgba(72, 49, 212, 0.04);
        transition: all 0.3s ease;
        flex-wrap: wrap;
        gap: 0.5rem;
    }
    ...
    .rowspan-border {
        border-right: 1px solid rgba(72, 49, 212, 0.05);
    }
    ...
    <div class="top-bar">
        ...
        <div class="rekap-switch">
            ...
            <div class="switch-group">
                <a href="?mode=harian<?= isset($_GET['kelas']) ? '&kelas='.$_GET['kelas'] : '' ?><?= isset($_GET['jurusan']) ? '&jurusan='.$_GET['jurusan'] : '' ?><?= isset($_GET['cari']) ? '&cari='.$_GET['cari'] : '' ?><?= $client_time ? '&client_time='.urlencode($client_time) : '' ?><?= isset($_GET['tanggal']) ? '&tanggal='.$_GET['tanggal'] : '' ?>" class="switch-btn <?php echo ($mode == 'harian') ? 'active' : ''; ?>">📅 Harian</a>
                <a href="?mode=mingguan<?= isset($_GET['kelas']) ? '&kelas='.$_GET['kelas'] : '' ?><?= isset($_GET['jurusan']) ? '&jurusan='.$_GET['jurusan'] : '' ?><?= isset($_GET['cari']) ? '&cari='.$_GET['cari'] : '' ?><?= $client_time ? '&client_time='.urlencode($client_time) : '' ?><?= isset($_GET['tanggal']) ? '&tanggal='.$_GET['tanggal'] : '' ?>" class="switch-btn <?php echo ($mode == 'mingguan') ? 'active' : ''; ?>">📆 Mingguan</a>
                <a href="?mode=bulanan<?= isset($_GET['kelas']) ? '&kelas='.$_GET['kelas'] : '' ?><?= isset($_GET['jurusan']) ? '&jurusan='.$_GET['jurusan'] : '' ?><?= isset($_GET['cari']) ? '&cari='.$_GET['cari'] : '' ?><?= $client_time ? '&client_time='.urlencode($client_time) : '' ?><?= isset($_GET['bulan']) ? '&bulan='.$_GET['bulan'] : '' ?><?= isset($_GET['tahun']) ? '&tahun='.$_GET['tahun'] : '' ?>" class="switch-btn <?php echo ($mode == 'bulanan') ? 'active' : ''; ?>">📊 Bulanan</a>
                ...
                <tbody>
                    ...
                    if (isset($conek) && $conek) {
                        ...
                        if (empty($absensi_data)) {
                            ...
                            // Kelompokkan data berdasarkan siswa dan tanggal
                            $grouped_data = [];
                            foreach ($absensi_data as $key => $data) {
                                ...
                                $grouped_data[$siswa_id]['data'][$tanggal] = $data;
                            }
...
<div class="modal-settings" id="settingsModal">
    <div class="modal-content">
        <div class="modal-header">
            <h2>⚙️ Pengaturan Jam Absen</h2>
            <button class="modal-close" onclick="closeSettingsModal()">✕</button>
        </div>

        <!-- FORM TAMBAH JAM -->
        <form method="POST" action="" class="form-settings" id="formJam">
            ...
            <div>
                ...
                <input type="text" name="nama_jam" id="nama_jam" placeholder="Contoh: Jam Masuk" required>
            </div>
            
            <div>
                <label for="urutan">Urutan</label>
                <input type="number" name="urutan" id="urutan" placeholder="1, 2, 3..." min="1" required>
            </div>
            ...
            <div class="full-width">
                <label for="batas_telat">Batas Telat (menit)</label>
                <input type="number" name="batas_telat" id="batas_telat" placeholder="Misal: 15 menit" min="0" required>
            </div>
            
            <div class="full-width" style="display: flex; gap: 0.5rem; margin-top: 0.5rem;">
                <button type="submit" class="btn-submit-settings" id="btnSubmitJam">➕ Tambah Jam</button>
                <button type="button" class="btn-cancel-settings" onclick="resetForm()">Batal</button>
            </div>
        </form>

        <!-- LIST JAM ABSEN -->
        <div class="jam-list">
            <h3>📋 Daftar Jam Absen</h3>
            <?php if (count($pengaturan_jam) > 0): ?>
                <?php 
                foreach ($pengaturan_jam as $jam):
                    $is_active = isJamAbsenActive($conek, $jam['jam_mulai'], $jam['jam_selesai'], $client_time_for_comparison);
                ?>
                    <div class="jam-item" id="jam-<?= $jam['id'] ?>">
                        <div class="info">
                            <span class="urutan">#<?= $jam['urutan'] ?></span>
                            <span class="nama"><?= htmlspecialchars($jam['nama_jam']) ?></span>
                            <span class="detail">🕐 <?= date('H:i', strtotime($jam['jam_mulai'])) ?> - <?= date('H:i', strtotime($jam['jam_selesai'])) ?></span>
                            <span class="detail">⏰ Batas Telat: <?= $jam['batas_telat'] ?> menit</span>
                            <?php if ($is_active): ?>
                        ...
                        <div class="actions">
                            <button class="btn-edit-jam" onclick="editJam(<?= $jam['id'] ?>, '<?= htmlspecialchars($jam['nama_jam']) ?>', '<?= $jam['jam_mulai'] ?>', '<?= $jam['jam_selesai'] ?>', <?= $jam['batas_telat'] ?>, <?= $jam['urutan'] ?>)">✏️ Edit</button>
                            <form method="POST" style="display: inline;" onsubmit="return confirm('Yakin hapus jam ini?')">
                                <input type="hidden" name="action" value="delete">
                                <input type="hidden" name="id" value="<?= $jam['id'] ?>">
                                <?php if ($client_time): ?>
                                    <input type="hidden" name="client_time" value="<?= htmlspecialchars($client_time) ?>">
                                <?php endif; ?>
                                <button type="submit" class="btn-delete-jam">🗑️ Hapus</button>
                            </form>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="empty-state" style="padding: 1rem;">
                    <span class="icon">📅</span>
                    <span class="text">Belum ada pengaturan jam absen</span>
                </div>
            <?php endif; ?>
        </div>
    </div>
...
// ===== EDIT JAM =====
function editJam(id, nama, mulai, selesai, batas, urutan) {
    document.getElementById('formAction').value = 'edit';
    document.getElementById('editId').value = id;
    document.getElementById('nama_jam').value = nama;
    document.getElementById('jam_mulai').value = mulai;
    document.getElementById('jam_selesai').value = selesai;
    document.getElementById('batas_telat').value = batas;
    document.getElementById('urutan').value = urutan;
    document.getElementById('btnSubmitJam').textContent = '💾 Update Jam';
    document.getElementById('btnSubmitJam').style.background = 'linear-gradient(135deg, #d97706, #fbbf24)';
    
    document.getElementById('formJam').scrollIntoView({ behavior: 'smooth', block: 'center' });
}
...
function resetForm() {
    ...
    document.getElementById('batas_telat').value = '';
    document.getElementById('urutan').value = '';
    document.getElementById('btnSubmitJam').textContent = '➕ Tambah Jam';
    document.getElementById('btnSubmitJam').style.background = 'linear-gradient(135deg, #4831d4, #6c63ff)';
}
...
<?php if ($has_active_jam): ?>
console.log('Active Jams:', <?= json_encode(array_column($active_jam_list, 'nama_jam')) ?>);
<?php endif; ?>
</script>
