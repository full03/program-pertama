...
if ($result->num_rows === 0) {
    header("Location: admin_dashboard.php?error=Data admin tidak ditemukan");
    exit();
}
...
// Cek apakah admin yang diedit adalah diri sendiri
$is_self = ($id == $_SESSION['user_id']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update') {
    ...
    if (empty($errors)) {
        // Cek apakah username/email sudah digunakan oleh admin lain
        $check_query = "SELECT id FROM admin WHERE (username = ? OR email = ?) AND id != ?";
        ...
        if ($check_result->num_rows > 0) {
            $update_error = "Username atau email sudah digunakan oleh admin lain!";
        } else {
            // Cek juga di tabel wali_kelas
            $check_wali = "SELECT id FROM wali_kelas WHERE username = ? OR email = ?";
            $check_wali_stmt = $conek->prepare($check_wali);
            $check_wali_stmt->bind_param("ss", $username, $email);
            $check_wali_stmt->execute();
            $check_wali_result = $check_wali_stmt->get_result();
            
            if ($check_wali_result->num_rows > 0) {
                $update_error = "Username atau email sudah digunakan oleh wali kelas!";
            } else {
                // Build query update
                if (!empty($password)) {
                ...
                if ($stmt->execute()) {
                    // Jika update diri sendiri, update session
                    if ($is_self) {
...
<head>
    ...
    <title>Edit Admin - Absensi RFID</title>
    ...
    <style>
        ...
        * {
        ...
        .edit-title {
            ...
            border-bottom: 2px solid rgba(72, 49, 212, 0.15);
        }
    ...
    <div class="header-modern">
        <div class="header-left">
            ...
            <div class="header-text">
                <h1>Edit Admin</h1>
                <div class="sub">Edit data admin yang terdaftar</div>
            </div>
        </div>
    ...
    <div class="edit-container">
        <div class="edit-card">
            <div class="edit-title">
                ...
                <?php if ($is_self): ?>
                    <span class="self-badge"><i class="fas fa-user-check"></i> Anda sendiri</span>
                <?php endif; ?>
            </div>
            ...
            <div class="info-box">
                ...
                <p>
                    ...
                    <?php if ($is_self): ?>
                        <br><i class="fas fa-exclamation-triangle" style="color:#dc2626;"></i> 
                        <strong style="color:#dc2626;">Perhatian:</strong> Anda sedang mengedit akun sendiri. Pastikan data yang dimasukkan benar.
                    <?php endif; ?>
                </p>
            </div>
...
console.log('✏️ Edit Admin siap!');
console.log('📝 Mengedit data admin ID: <?php echo $admin['id']; ?>');
console.log('👤 Nama: <?php echo htmlspecialchars($admin['nama_lengkap'] ?? ''); ?>');
<?php if ($is_self): ?>
console.log('⚠️ Anda sedang mengedit akun sendiri');
<?php endif; ?>
</script>
