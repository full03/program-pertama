...
// Cek role - hanya wali_kelas yang bisa mengakses
...
// ===== AMBIL KELAS WALI DARI SESSION =====
$kelas_wali = $_SESSION['kelas'] ?? '';

// Jika tidak ada kelas di session, coba ambil dari database
if (empty($kelas_wali)) {
...
// ===== FILTER WAJIB: KELAS WALI =====
if (!empty($kelas_wali)) {
    $filter_kelas = $kelas_wali;
}
...
if ($tanggal_awal_filter && $tanggal_akhir_filter) {
    ...
    $query .= " AND (r.tanggal BETWEEN '$tanggal_awal_clean' AND '$tanggal_akhir_clean' OR r.tanggal IS NULL) ";

    // FILTER WAJIB: Kelas wali
    if (!empty($kelas_wali)) {
        $kelas_clean = mysqli_real_escape_string($conek, $kelas_wali);
        $query .= " AND s.kelas = '$kelas_clean' ";
    }

    // Filter kelas tambahan (jika ada dan berbeda dari kelas wali)
    if (!empty($filter_kelas) && $filter_kelas != $kelas_wali) {
        $kelas_clean = mysqli_real_escape_string($conek, $filter_kelas);
        $query .= " AND s.kelas = '$kelas_clean' ";
    }

    // Filter jurusan
    if (!empty($filter_jurusan)) {
        $jurusan_clean = mysqli_real_escape_string($conek, $filter_jurusan);
    ...
    // Cek error query
    if (!$sql) {
        $error_message = "Error query: " . mysqli_error($conek);
    } else {
...
// ===== AMBIL DAFTAR KELAS (HANYA UNTUK WALI KELAS) =====
$daftar_kelas = [];
if (!empty($kelas_wali)) {
    $daftar_kelas[] = $kelas_wali;
} else {
    // Jika bukan wali kelas, ambil semua kelas
...
// ===== AMBIL DAFTAR JURUSAN (HANYA UNTUK KELAS WALI) =====
$daftar_jurusan = [];
if (!empty($kelas_wali)) {
    $qJurusan = mysqli_query($conek, "SELECT DISTINCT jurusan FROM siswa WHERE kelas = '$kelas_wali' ORDER BY jurusan ASC");
    if ($qJurusan) {
        while ($j = mysqli_fetch_assoc($qJurusan)) {
            $daftar_jurusan[] = $j['jurusan'];
        }
    }
} else {
...
<head>
    ...
    * {
    ...
    .filter-inline .btn-filter:hover {
        ...
        box-shadow: 0 6px 25px rgba(72, 49, 212, 0.25);
    }
    ...
    .table-footer-stats {
        ...
        border-top: 2px solid rgba(245, 158, 11, 0.06);
        padding: 0.5rem 1rem;
        display: flex;
...
function editStatusRowModal(nokartu, tanggal, statusSekarang, rowId) {
    ...
    html += '<select id="edit_status_' + rowId + '" class="edit-select">';
    html += optionsHtml;
    html += '</select>';
...
<div class="container-fluid">
    ...
    <div class="header-modern">
            ...
            <div class="header-text">
                ...
                <div class="sub">
                    ...
                    <?php if (!empty($filter_jurusan)): ?>
                        <span style="background: rgba(108,99,255,0.08); padding: 2px 12px; border-radius: 12px; font-weight: 600; color: #6c63ff; margin-left: 4px;">
                            <i class="fas fa-book"></i> <?php echo htmlspecialchars($filter_jurusan); ?>
                        </span>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="header-stats">
            ...
            <div class="stat-chip" style="border-right:1px solid rgba(72,49,212,0.10); padding-right:1rem;">
                <span class="number"><?= $total_siswa_filtered ?></span>
            ...
            <div class="stat-chip" style="border-right:1px solid rgba(72,49,212,0.10); padding-right:1rem;">
                <span class="number">
                    ...
                    if (isset($conek) && $conek) {
                        ...
                        if(!empty($kelas_wali)){
                            $kelas = mysqli_real_escape_string($conek, $kelas_wali);
                            $filterKelas = " AND kelas='$kelas' ";
                        }
                        if(!empty($filter_jurusan)){
                            $jurusan = mysqli_real_escape_string($conek, $filter_jurusan);
    ...
    <div class="top-bar">
        <div class="rekap-switch">
            ...
            <div class="switch-group">
                <a href="?mode=harian<?= !empty($kelas_wali) ? '&kelas='.urlencode($kelas_wali) : '' ?><?= !empty($filter_jurusan) ? '&jurusan='.urlencode($filter_jurusan) : '' ?><?= isset($_GET['tanggal']) ? '&tanggal='.$_GET['tanggal'] : '' ?><?= !empty($search_nama) ? '&search_nama='.urlencode($search_nama) : '' ?><?= $client_time ? '&client_time='.urlencode($client_time) : '' ?>" class="switch-btn <?php echo (!isset($_GET['mode']) || $_GET['mode']=='harian') ? 'active' : ''; ?>">📅 Harian</a>
                <a href="?mode=mingguan<?= !empty($kelas_wali) ? '&kelas='.urlencode($kelas_wali) : '' ?><?= !empty($filter_jurusan) ? '&jurusan='.urlencode($filter_jurusan) : '' ?><?= isset($_GET['tanggal']) ? '&tanggal='.$_GET['tanggal'] : '' ?><?= !empty($search_nama) ? '&search_nama='.urlencode($search_nama) : '' ?><?= $client_time ? '&client_time='.urlencode($client_time) : '' ?>" class="switch-btn <?php echo (isset($_GET['mode']) && $_GET['mode']=='mingguan') ? 'active' : ''; ?>">📆 Mingguan</a>
                <a href="?mode=bulanan<?= !empty($kelas_wali) ? '&kelas='.urlencode($kelas_wali) : '' ?><?= !empty($filter_jurusan) ? '&jurusan='.urlencode($filter_jurusan) : '' ?><?= isset($_GET['bulan']) ? '&bulan='.$_GET['bulan'] : '' ?><?= isset($_GET['tahun']) ? '&tahun='.$_GET['tahun'] : '' ?><?= !empty($search_nama) ? '&search_nama='.urlencode($search_nama) : '' ?><?= $client_time ? '&client_time='.urlencode($client_time) : '' ?>" class="switch-btn <?php echo (isset($_GET['mode']) && $_GET['mode']=='bulanan') ? 'active' : ''; ?>">📊 Bulanan</a>
                <a href="?mode=semester<?= !empty($kelas_wali) ? '&kelas='.urlencode($kelas_wali) : '' ?><?= !empty($filter_jurusan) ? '&jurusan='.urlencode($filter_jurusan) : '' ?><?= isset($_GET['semester']) ? '&semester='.$_GET['semester'] : '' ?><?= isset($_GET['tahun']) ? '&tahun='.$_GET['tahun'] : '' ?><?= !empty($search_nama) ? '&search_nama='.urlencode($search_nama) : '' ?><?= $client_time ? '&client_time='.urlencode($client_time) : '' ?>" class="switch-btn <?php echo (isset($_GET['mode']) && $_GET['mode']=='semester') ? 'active' : ''; ?>">🎓 Semester</a>
            </div>
                    ...
                    <select name="jurusan">
                        ...
                        <?php foreach ($daftar_jurusan as $jurusan): ?>
                            <option value="<?php echo htmlspecialchars($jurusan); ?>" 
                                <?php echo ($filter_jurusan == $jurusan) ? 'selected' : ''; ?>>
    ...
    <div style="padding: 0.5rem 1.2rem; background: rgba(255,255,255,0.6); border-radius: 0; margin-bottom: 0.1rem; display: flex; align-items: center; gap: 0.8rem; flex-wrap: wrap;">
        ...
        if (!empty($search_nama)) {
            echo '<span style="background: rgba(245, 158, 11, 0.08); padding: 0.2rem 0.8rem; border-radius: 20px; font-size: 0.65rem; color: #b45309; border: 1px solid rgba(245, 158, 11, 0.10);">
    ...
    <div class="table-wrapper">
                ...
                <tbody>
                    ...
                    if (isset($conek) && $conek) {
                        if (!$sql) {
                            ...
                            if (!empty($kelas_wali)) {
                                $mode_detail .= ' di Kelas ' . htmlspecialchars($kelas_wali);
                            }
                            ...
                            echo '<tr><td colspan="9" class="empty-state">
                                ...
                                <div class="sub-text">Silahkan lakukan absensi terlebih dahulu</div>
                            </td></tr>';
                        } else {
                            $no = 1;
                    ...
                    <tr>
                        ...
                        <td style="font-weight:600; color:#1a1a2e; font-size:0.7rem;">
                            ...
                            <?php if (!empty($search_nama)): ?>
                                <span style="background: rgba(245, 158, 11, 0.08); padding: 1px 6px; border-radius: 10px; font-size: 0.5rem; color: #b45309; margin-left: 4px;">✓</span>
                            <?php endif; ?>
                        </td>
                        <td class="center">
        ...
        <div class="table-footer-stats">
            ...
            <div class="stat-item">
                ...
                <span class="value" style="font-weight:700; color:#b45309;"><?= $total_hadir + $total_sakit + $total_izin + $total_alfa ?></span>
            </div>
            <?php if (!empty($search_nama)): ?>
            <div class="stat-item">
                <span class="label">🔍 Menampilkan hasil untuk:</span>
                <span class="value" style="font-weight:600; color:#b45309; font-size:0.7rem;">"<?= htmlspecialchars($search_nama) ?>"</span>
            </div>
            <?php endif; ?>
            <?php if (!empty($kelas_wali)): ?>
            <div class="stat-item">
                <span class="label">📚 Kelas:</span>
                <span class="value" style="font-weight:600; color:#b45309; font-size:0.7rem;"><?= htmlspecialchars($kelas_wali) ?></span>
            </div>
            <?php endif; ?>
        </div>
        <?php endif; ?>
